...
$script:SelectedAgentRuntime = $null
...
function Ensure-AgentRuntime {
  Add-ProcessPathItem $root
  if (-not (Test-Path (Join-Path $root 'codex.cmd'))) {
    throw "Missing local router shim: $(Join-Path $root 'codex.cmd')"
  }
  Write-Host "Agent shim ready: $(Join-Path $root 'codex.cmd')"
}

function Get-AgentRuntime {
  if ($script:SelectedAgentRuntime) {
    return $script:SelectedAgentRuntime
  }

  $shimPath = Join-Path $root 'codex.cmd'
  if (-not (Test-Path $shimPath)) {
    throw "Missing local router shim: $shimPath"
  }

  Add-ProcessPathItem $root
  $script:SelectedAgentRuntime = [pscustomobject]@{
    Name = 'Router Shim'
    Type = 'codex'
    Mode = 'full-auto'
    Command = $shimPath
...
function Ensure-WeixinConfig {
  ...
  if ([string]::IsNullOrWhiteSpace([string]$current['WEIXIN_CDN_BASE_URL'])) {
    $current['WEIXIN_CDN_BASE_URL'] = 'https://novac2c.cdn.weixin.qq.com/c2c'
  }
...
$useSavedConfig = [string]::Equals([string]$env:LAUNCH_ROUTER_USE_SAVED, '1')
$runForeground = [string]::Equals([string]$env:LAUNCH_ROUTER_FOREGROUND, '1')

if (Test-Path $envFile) {
  Write-Host 'Saved config found.'
  if (-not $useSavedConfig) {
    ...
    Write-Host 'Using saved config automatically.'
  }
}
...
if ($runForeground) {
  ...
  $startOptions['RedirectStandardOutput'] = $routeOutPath
  $startOptions['RedirectStandardError'] = $routeErrPath
}

$routeProcess = Start-Process @startOptions
if (-not $routeProcess) {
  Clear-TrackedProcessIds
...
if ($null -eq $routeExitCode) {
  $routeExitCode = 1
}

if ($routeExitCode -ne 0 -and -not $runForeground) {
